...
```{r rnt_bmigrs-bmi, echo = FALSE, fig.width = 8, fig.height = 1.5}
w = grep("_GRS", colnames(mydata))
grs_vars = colnames(mydata)[w]
##
...
```{r met_BMI_cors, fig.width = 12, fig.height = 5, warning=FALSE, error=FALSE, message=FALSE}
...
ds = sapply( visit_cors$metabolite[o[1:100]], function(x){ strsplit( x , split = "_")[[1]][2] })
perc_response = (length( grep( "r", ds ) ) / 100) *100
###
