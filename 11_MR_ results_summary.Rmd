...
## What is the correlation between female and male effect estimates

```{r}
cor.test(obs_tsls_est$wNEO_female$MR_beta, obs_tsls_est$wNEO_male$MR_beta)$p.value
```


```{r}
temp_plotdata = data.frame(female = obs_tsls_est$wNEO_female$MR_beta,
                           male = obs_tsls_est$wNEO_male$MR_beta)
temp_plotdata %>% ggplot(aes(x = female, y = male)) +
  geom_point( fill = "dodgerblue", size = 5, shape = 21, alpha = 0.7) +
...
## correlation among sexes is poor but are any estimates actually different from each other. 

```{r}
temp = data.frame( outcome = obs_tsls_est$wNEO_female$outcome,
                   female = obs_tsls_est$wNEO_female$MR_beta,
                   female_se = obs_tsls_est$wNEO_female$MR_se,
                   female_P = obs_tsls_est$wNEO_female$MR_P,
                   male = obs_tsls_est$wNEO_male$MR_beta,
                   male_se = obs_tsls_est$wNEO_male$MR_se,
                   male_P = obs_tsls_est$wNEO_male$MR_P )
...
temp$sig_dif[q] = "P<0.05/687"
temp$sig_dif = factor(temp$sig_dif, levels = c("P>0.05", "P<0.05", "P<0.05/687" )[3:1] )
...
## BF significant MR results in the sample population ordered by dietary state and P

```{r}
mytable = obs_tsls_est$wNEO %>% filter( MR_P < 0.05/43 ) %>% 
  arrange(dietary_state, MR_P) %>%
    dplyr::select(outcome, raw.label, class, dietary_state,
                  Obs_W, Obs_beta, Obs_se, Obs_P, 
                  MR_Wstat, MR_beta, MR_se, MR_P) 
...
```{r}
mytable = obs_tsls_est$wNEO %>% filter( MR_P < 0.05/43, dietary_state == "response" ) %>% 
  arrange( MR_P) %>%
    dplyr::select(outcome, raw.label, class, dietary_state,
                  Obs_W, Obs_beta, Obs_se, Obs_P, 
                  MR_Wstat, MR_beta, MR_se, MR_P) 
...
```{r}
shapiro.test(obs_tsls_est$wNEO_female$MR_beta)
shapiro.test( abs(obs_tsls_est$wNEO_female$MR_beta) )

shapiro.test(obs_tsls_est$wNEO_male$MR_beta)
```



```{r}
quantile(obs_tsls_est$wNEO_female$MR_beta, probs = c(0.025, 0.5, 0.975))
quantile(obs_tsls_est$wNEO_male$MR_beta, probs = c(0.025, 0.5, 0.975))
```

```{r}
summary( obs_tsls_est$wNEO_female$MR_beta )
summary( obs_tsls_est$wNEO_male$MR_beta )

t.test(obs_tsls_est$wNEO_female$MR_beta, obs_tsls_est$wNEO_male$MR_beta, paired = TRUE)
t.test( abs(obs_tsls_est$wNEO_female$MR_beta), abs(obs_tsls_est$wNEO_male$MR_beta), paired = TRUE)$p.value
t.test( abs(obs_tsls_est$wNEO_female$MR_beta), abs(obs_tsls_est$wNEO_male$MR_beta), paired = FALSE)
var.test(obs_tsls_est$wNEO_female$MR_beta, obs_tsls_est$wNEO_male$MR_beta)
```


```{r}
w = which(obs_tsls_est$wNEO_male$derived_features == "yes")
t.test( abs(obs_tsls_est$wNEO_female$MR_beta[w]), abs(obs_tsls_est$wNEO_male$MR_beta[w]), paired = FALSE)
```



```{r}
hist(obs_tsls_est$wNEO_female$MR_beta)
hist(obs_tsls_est$wNEO_male$MR_beta)
```
...
```{r}
zt = function(beta1, se1, beta2, se2){
...
i = 226
zt(beta1 = temp[i,2], se1 = temp[i,3] , beta2 = temp[i,5] , se2 = temp[i,6] ) 
ztest(beta1 = temp[i,2], se1 = temp[i,3] , beta2 = temp[i,5] , se2 = temp[i,6] ) 
```
...
###
# f = grep( "_f", obs_tsls_est$wNEO$outcome )
f = which( obs_tsls_est$wNEO$dietary_state == "fasting" )
# p = grep( "_p", obs_tsls_est$wNEO$outcome )
p = which( obs_tsls_est$wNEO$dietary_state == "postprandial" )
r = which( obs_tsls_est$wNEO$dietary_state == "response" )
...
temp = data.frame( outcome = obs_tsls_est$wNEO$outcome[f],
                   ...
                   postprandial_se = obs_tsls_est$wNEO$MR_se[p],
                   postprandial_P = obs_tsls_est$wNEO$MR_P[p],
                   response = obs_tsls_est$wNEO$MR_beta[r],
                   response_se = obs_tsls_est$wNEO$MR_se[r],
                   response_P = obs_tsls_est$wNEO$MR_P[r])
temp$outcome = gsub("_f","",temp$outcome )
###
PearsonR_fp = cor.test(temp$fasting, temp$postprandial)$estimate
PearsonR_fp = formatC(PearsonR, digits = 3)

PearsonR_fr = cor.test(temp$fasting, temp$response)$estimate
PearsonR_fr = formatC(PearsonR_fr, digits = 3)

PearsonR_pr = cor.test(temp$postprandial, temp$response)$estimate
PearsonR_pr = formatC(PearsonR_pr, digits = 3)
...
###
p2 = temp %>% ggplot(aes(x = fasting, y = postprandial)) +
  geom_vline(xintercept = 0, linetype = "dashed", color = "grey50") +
  geom_hline(yintercept = 0, linetype = "dashed", color = "grey50") +
  geom_smooth( method = "lm", formula = y ~ x , color = "black", size = 3) +
  geom_abline(intercept = 0, slope = 1, linetype = "dashed", color = "red") +
