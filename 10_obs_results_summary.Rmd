...
```{r}
f =  which( obs_tsls_est$wNEO$Obs_P < 0.05/43 & obs_tsls_est$wNEO$dietary_state == "fasting")
f = obs_tsls_est$wNEO$outcome[f]
f = gsub("_f","",f)
p =  which( obs_tsls_est$wNEO$Obs_P < 0.05/43 & obs_tsls_est$wNEO$dietary_state == "postprandial")
p = obs_tsls_est$wNEO$outcome[p]
p = gsub("_p","",p)
```
...
## observational fasting vs observationl postprandial cor.
...
##does response correlate with either fasting or postprandial data?

```{r}
cor.test(fdata$fasting_Obs_beta, rdata$response_Obs_beta)
```


```{r}
cor.test(pdata$postprandial_Obs_beta, rdata$response_Obs_beta)
```
...
```{r}
plot_traits = c("ala_r","leu_r")
AA_stats = t( sapply(plot_traits, function(trait){
